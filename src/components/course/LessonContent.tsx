  ...
  return <div ref={ref} className="min-h-screen">
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b border-border px-4 py-3">
        <div className="flex items-center gap-4">
          <button onClick={onToggleSidebar} className="p-2 rounded-lg hover:bg-muted transition-colors" aria-label="Toggle sidebar">
      ...
      <div className="notebook-paper min-h-[calc(100vh-60px)]">
        <motion.div key={lesson.id} initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        duration: 0.3
          ...
          <div className="notebook-block py-12">
            ...
            <div className="flex items-center justify-between pt-8 border-t border-border">
              <Button variant="outline" className="gap-2" disabled={!previousLesson} onClick={() => previousLesson && onNavigate(previousLesson.id)}>
                <ChevronLeft className="w-4 h-4" />
                Previous Lesson
              </Button>
              
              {isLastLessonInCourse ? <Button className="gap-2" onClick={() => {
              onFinishCourse();
            }}>
                  <Trophy className="w-4 h-4" />
                  Finish Course
                </Button> : isLastLessonInModule ? <Button className="gap-2" onClick={() => {
              onOpenQuiz();
            }}>
                  <ClipboardList className="w-4 h-4" />
                  Attend Quiz
                </Button> : <Button className="gap-2" disabled={!nextLesson} onClick={() => nextLesson && onNavigate(nextLesson.id, {
              completeLessonId: lesson.id
            })}>
                  Next Lesson
                  <ChevronRight className="w-4 h-4" />
                </Button>}
